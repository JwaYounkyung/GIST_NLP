...
class Field(RawField):
    ...
    def build_vocab_exceptunk(self, *args, **kwargs):
        """Construct the Vocab object for this field from one or more datasets.
        ...
        for data in sources:
            for x in data:
                ...
                try:
                    ...
                    counter.update(chain.from_iterable(x))
        specials = list(OrderedDict.fromkeys(
            tok for tok in [self.pad_token, self.init_token,
                            self.eos_token] + kwargs.pop('specials', [])
            if tok is not None))
        self.vocab = self.vocab_cls(counter, specials=specials, **kwargs)
...
class OneHotField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        """
        padded = self.pad(batch)
        tensor = self.numericalize_onehot(padded, device=device)
        return tensor
    ...
    def numericalize_onehot(self, arr, device=None):
        """Turn a batch of examples that use this field into a Variable.
        ...
        if self.use_vocab:
            if self.sequential:
                arr = [[one_hot_encoding(x) for x in ex] for ex in arr]
            else:
                arr = [one_hot_encoding(x) for x in arr]
