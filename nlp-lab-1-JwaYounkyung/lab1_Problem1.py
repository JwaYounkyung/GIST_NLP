...
TEXT = legacy.data.OneHotField(sequential=True, batch_first=True, fix_length=20)
...
model = TextClassificationModel(vocab_size, text_len, n_classes).to(DEVICE)
model.float()
...
for epoch in range(1, EPOCHS+1):
    ...
    if best_train_acc is not None and best_train_acc > train_acc: # accuracy가 업데이트가 안되었을 때
        ...
        if not os.path.isdir("results"):
            os.makedirs("results")
        torch.save(model.state_dict(), './results/lab1_Problem1.pt')
...
graph(train_loss_list, 'loss_Problem1')
graph(train_acc_list, 'acc_Problem1')
...
def generator(test_iter):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(test_iter):
            ...
            logit = model(x.float()) # Text.vocab update도 이루어짐
            output.extend(logit.argmax(1))

    return output

model.load_state_dict(torch.load('./results/lab1_Problem1.pt',  map_location=DEVICE))
...
result_df.to_csv("results/result_lab1_Problem1.csv", index=False)